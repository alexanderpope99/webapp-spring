...
public interface RealizariRetineriRepository extends JpaRepository<RealizariRetineri, Long> { 
		...
		public long getSalDatoratByLunaAndAn(int luna, int an, long idsocietate);
		...
		public long getCAS25ByLunaAndAn(int luna, int an, long idsocietate);
		...
		public long getCASS10ByLunaAndAn(int luna, int an, long idsocietate);
		...
		public long getImpozitByLunaAndAn(int luna, int an, long idsocietate);
		...
		public long getValCMByLunaAndAn(int luna, int an, long idsocietate);
		...
		public long getCAMByLunaAndAn(int luna, int an, long idsocietate);

		@Query(value = "select sum(cas) as cas25, sum(cass) as cass10, sum(impozit) as impozit, sum(salariurealizat) as salariuDatorat, sum(valcm) as valCM, sum(cam) as cam from realizariretineri where luna = ?1 and an = ?2 and idcontract in (select id from contract where id in (select idcontract from angajat where idsocietate = ?3))",
		nativeQuery = true)
		public NotaContabila getNotaContabilaByLunaAndAnAndIdsocietate(int luna, int an, long idsocietate);


    public boolean existsByLunaAndAnAndIdcontract(int luna, int an, long idcontract);
}

interface NotaContabila {
	long cas25();
	long cass10();
	long impozit();
	long salariuDatorat();
