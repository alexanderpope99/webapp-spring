...
class Contract extends React.Component {
  constructor(props) {
    ...
    if (typeof props.location !== 'undefined') {
      let search = props.location.search; // URL
  ...
  async onSubmit(e, met) {
    ...
    const contract = await fetch(
      `http://localhost:5000/contract/${this.state.selected === -1 ? '' : this.state.selected}`,
      {
        method: met, // function parameter :: is 'PUT' or 'POST'
        headers: { 'Content-Type': 'application/json' },
    ...
    if (typeof this.props.asChild === 'undefined') {
      if (typeof contract.id === 'number') {
        ...
        this.setState({
          show: true,
          modalMessage: met === 'POST' ? 'Contract adăugat cu succes.' : 'Contract actualizat.',
        });
          ...
          <Col>
            <Card>
              <Card.Header style={{ border: 'none', paddingBottom: '0rem' }}>
                <Card.Title as="h4">Contract</Card.Title>
              ...
              <Card.Body>
                <Form>
                  ...
                  {typeof this.props.asChild === 'undefined' ? (
                    <Row>
                      <Col md={6}>
                        <Button variant="outline-primary" onClick={(e) => this.onSubmit(e, 'POST')}>
                          Adaugă
                        </Button>
                      </Col>
                      <Col md={6}>
