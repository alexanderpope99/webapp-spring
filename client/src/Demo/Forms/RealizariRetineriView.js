...
class RealizariRetineriView extends React.Component {
  constructor() {
    ...
    this.setLastYearMonth = this.setLastYearMonth.bind(this);
  ...
  async componentDidMount() {
    ...
    this.setLastYearMonth(); // modifies state.an, state.luna
  ...
  async setLastYearMonth() {
    ...
    const lunisiani = await axios
      .get(`${server.address}/realizariretineri/idc=${contract.id}`, {
        headers: authHeader(),
      })
      .then((res) => (res.status === 200 ? res.data : null))
      .catch((err) => console.error(err));
    ...
    this.setState({
      lunaan: lunisiani,
      an: lunisiani[lunisiani.length - 1].split(';')[2],
      luna: {
        nume: lunisiani[lunisiani.length - 1].split(';')[0],
        nr: lunisiani[lunisiani.length - 1].split(';')[1],
      },
    });
  }
  ...
  render() {
    var ani = [];
    this.state.lunaan.forEach(function (el) {
      ani.push(<option key={el.split(';')[2]}>{el.split(';')[2]}</option>);
    });

    var luniComponent = [];
    this.state.lunaan.forEach(function (el) {
      if (el.split(';')[1] === this.state.an)
        luniComponent.push(
          <option key={el.split(';')[0]} data-key={el.split(';')[0] + 1}>
            {el.split(';')[0]}
          </option>
        );
    });
